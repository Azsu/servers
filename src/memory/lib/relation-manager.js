...
import { EntityNotFoundError } from './types.js';
/**
...
export class RelationManager extends GraphOperations {
     ...
     */
    async createExperienceSkillRelation(relation) {
     ...
     * @throws {EntityNotFoundError} If referenced entities don't exist
     */
    async createProjectExperienceRelation(relation) {
        const graph = await this.loadGraph();
     ...
     * @throws {EntityNotFoundError} If referenced entities don't exist
     */
    async createCrossReferenceRelation(relation) {
        const graph = await this.loadGraph();
     ...
     * @throws {EntityNotFoundError} If referenced entities don't exist
     */
    async createContributionExperienceRelation(relation) {
        const graph = await this.loadGraph();
     ...
     */
    async getSkillsForExperience(experienceId) {
        const graph = await this.loadGraph();
     ...
     */
    async getProjectsForExperience(experienceId) {
        const graph = await this.loadGraph();
        return graph.relations
    ...
    async getRelatedEntities(entityId, entityType) {
        ...
        return graph.relations
            ...
            sourceId: r.from,
            targetType: r.to === entityId ? entityType : "RELATED",
            targetId: r.to,
     ...
     */
    async getContributionsForExperience(experienceId) {
        const graph = await this.loadGraph();
        return graph.relations
