...
export class EntityManager extends GraphOperations {
     ...
     */
    async createEntityOfType(entityType, name, description = "") {
        const entity = {
            name,
            entityType,
     ...
     */
    async searchEntitiesByType(entityType, query) {
     ...
     */
    async addWorkExperience(experiences) {
        ...
        return experiences.filter(exp => createdEntities.some(entity => entity.name === exp.title));
    }
    /**
     * Deletes work experiences from the graph
     * @param titles Array of work experience titles to delete
     */
    async deleteWorkExperience(titles) {
     ...
     */
    async searchWorkExperience(query) {
     ...
     */
    async addEducationExperience(experiences) {
        const entities = experiences.map(exp => ({
            name: exp.school,
        ...
        const createdEntities = await this.createEntities(entities);
        return experiences.filter(exp => createdEntities.some(entity => entity.name === exp.school));
    }
    /**
     * Deletes education experiences from the graph
     * @param schools Array of school names to delete
     */
    async deleteEducationExperience(schools) {
     ...
     */
    async searchEducationExperience(query) {
    ...
    async addCareerBreak(breaks) {
        ...
        const createdEntities = await this.createEntities(entities);
        return breaks.filter(br => createdEntities.some(entity => entity.name === br.type));
    }
    /**
     * Deletes career breaks from the graph
     * @param types Array of career break types to delete
     */
    async deleteCareerBreak(types) {
    ...
    async addProject(projects) {
        ...
        const createdEntities = await this.createEntities(entities);
        return projects.filter(proj => createdEntities.some(entity => entity.name === proj.projectName));
    }
    /**
     * Deletes projects from the graph
     * @param projectNames Array of project names to delete
     */
    async deleteProject(projectNames) {
    ...
    async addCourse(courses) {
        ...
        const createdEntities = await this.createEntities(entities);
        return courses.filter(course => createdEntities.some(entity => entity.name === course.courseName));
    }
    /**
     * Deletes courses from the graph
     * @param courseNames Array of course names to delete
     */
    async deleteCourse(courseNames) {
    ...
    async addReference(references) {
        ...
        const createdEntities = await this.createEntities(entities);
        return references.filter(ref => createdEntities.some(entity => entity.name === ref.name));
    }
    /**
     * Deletes references from the graph
     * @param names Array of reference names to delete
     */
    async deleteReference(names) {
    ...
    async addOrganization(orgs) {
        ...
        const createdEntities = await this.createEntities(entities);
        return orgs.filter(org => createdEntities.some(entity => entity.name === org.organizationName));
    }
    /**
     * Deletes organizations from the graph
     * @param names Array of organization names to delete
     */
    async deleteOrganization(names) {
        await this.deleteEntities(names);
    }
    /**
    ...
    async addHonorOrAward(items) {
        ...
        const createdEntities = await this.createEntities(entities);
        return items.filter(item => createdEntities.some(entity => entity.name === item.title));
    }
    /**
     * Deletes honors or awards from the graph
     * @param titles Array of honor or award titles to delete
     */
    async deleteHonorOrAward(titles) {
        await this.deleteEntities(titles);
    }
    /**
     ...
     */
    async searchHonorOrAward(query) {
