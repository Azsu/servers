...
import { LicenseOrCertificationSchema, OnlineContributionSchema, ValidationError } from './types.js';
/**
...
export class ContributionsManager extends GraphOperations {
     ...
     */
    async addOnlineContribution(contributions) {
        ...
        return contributions.filter(contribution => createdEntities.some(entity => entity.name === contribution.title));
    }
    /**
     * Deletes online contributions from the graph
     * @param titles Array of contribution titles to delete
     */
    async deleteOnlineContribution(titles) {
     ...
     */
    async searchOnlineContribution(query) {
     ...
     */
    async addLicenseOrCertification(items) {
        ...
        const createdEntities = await this.createEntities(entities);
        return items.filter(item => createdEntities.some(entity => entity.name === item.name));
    }
    /**
     * Deletes licenses or certifications from the graph
     * @param names Array of license/certification names to delete
     */
    async deleteLicenseOrCertification(names) {
     ...
     */
    async searchLicenseOrCertification(query) {
        const graph = await this.loadGraph();
        return graph.entities
