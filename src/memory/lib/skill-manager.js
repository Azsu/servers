...
import { EntityNotFoundError, SkillSchema, ValidationError } from './types.js';
/**
...
export class SkillManager extends GraphOperations {
    ...
    async addSkill(skills) {
        ...
        const entities = skills.map((skill) => ({
        ...
        return skills.filter(skill => createdEntities.some((entity) => entity.name === skill.skillName));
    }
    /**
     * Deletes skills from the graph
     * @param skillNames Array of skill names to delete
     */
    async deleteSkill(skillNames) {
     ...
     */
    async calculateExperience(technology) {
        const graph = await this.loadGraph();
        let totalDuration = 0;
        const techEntity = graph.entities.find(e => e.name.toLowerCase() === technology.toLowerCase() &&
            (e.entityType === "Skill" || e.entityType === "Technology"));
        if (!techEntity) {
            throw new EntityNotFoundError(technology);
        }
        // Calculate from relations
        const relatedExperiences = graph.relations.filter(relation => relation.from === techEntity.name &&
     ...
     * @throws {ValidationError} If skill data parsing fails
     */
    async calculateSkillMetrics(skillName) {
        const graph = await this.loadGraph();
        const skillEntity = graph.entities.find(e => e.entityType === "Skill" && e.name === skillName);
        if (!skillEntity) {
        ...
        try {
            ...
            const dates = workExperiences
                .map((exp) => {
                const workExp = graph.entities.find(e => e.entityType === "WorkExperience" && e.name === exp.experienceId);
                if (!workExp)
                    return null;
                try {
            ...
            // Calculate metrics
            const firstUsed = new Date(Math.min(...dates.map((d) => d.start.getTime()))).toISOString();
            const lastUsed = new Date(Math.max(...dates.map((d) => d.end.getTime()))).toISOString();
            const totalYearsExperience = dates.reduce((total, date) => {
